...
test('creates a new question when the form is submitted', async () => {
  ...
  fireEvent.change(screen.queryByLabelText(/Correct Index/), {
    target: { value: '1' },
  });
  fireEvent.submit(screen.queryByText(/New Question/));
  fireEvent.click(screen.queryByText(/View Questions/));
...
test('updates the answer when the dropdown is changed', async () => {
  ...
  fireEvent.click(screen.queryByText(/View Questions/));
  await screen.findByText(/lorem testum 2/g);
  fireEvent.change(screen.queryAllByLabelText(/Correct Index/)[0], {
    target: { value: '3' },
  });
  expect(screen.queryAllByLabelText(/Correct Index/)[0].value).toBe('3');
  rerender(<App />);
  expect(screen.queryAllByLabelText(/Correct Index/)[0].value).toBe('3');
});
